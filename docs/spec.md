...
- `vue-i18n` 管理 zh/en/fr/ru/ar 五种本地语言包；全局语言选择、系统语言识别、英文回退及本机偏好保存。
- 覆盖全部功能页、公共弹窗、动态提示、可访问名称及日期/数量显示。阿拉伯文使用 RTL，代码、路径和文档预览分别保持适合内容的文本方向。
- 翻译层不参与文档、规则或映射持久化；内置规则只在原始默认名称/描述未修改时本地化显示，自定义内容原样呈现。外部发布说明和原始诊断不自动翻译。
- `npm run check:i18n` 校验五语言键集合、插值参数、消息语法和 Vue 模板。
