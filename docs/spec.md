...
phone、id_card、bank_card 三项始终保留于管理列表，旧删除标记忽略。导入或批量删除不能移除这些字段，启停状态仍由用户管理。删除全部可删除项后列表至少保留这三项；停用全部仍返回零自动检测结果。

## 自动检测规则来源

自动检测严格使用敏感字段管理中当前启用的配置。规则为空或全部停用时，上传不产生自动候选。正则、关键词、姓名按配置值匹配；内置算法仅在对应配置启用时执行校验。未接入的 NLP 或图片识别不生成模拟候选，手动操作和主动 AI 调用保持独立。

## 敏感字段批量管理

批量编辑支持删除所选规则。删除前必须应用内确认，内置项同步写入删除标记，成功后刷新列表、计数并清空选择。

默认以 🟢/🔴 标记规则启用状态，不显示复选框。通过正则转换左侧的编辑按钮进入批量模式，支持单项选择、全选、批量停用和批量启用；选择独立于规则启用状态，完成后清空选择。现有单条规则内容编辑保留。

## 前端国际化（2026-09-09）

- `vue-i18n` 仅管理 zh/en/fr/ja/de/ko 六种本地语言包；全局语言选择、系统语言识别、英文回退及本机偏好保存。旧版 ru/ar 偏好迁移到受支持的系统语言，无匹配时回退 en。
- 覆盖全部功能页、公共弹窗、动态提示、可访问名称及日期/数量显示。六种界面语言均使用 LTR，文档预览保持适合其内容的文本方向。
- 翻译层不参与文档、规则或映射持久化；内置规则只在原始默认名称/描述未修改时本地化显示，自定义内容原样呈现。外部发布说明和原始诊断不自动翻译。
- `npm run check:i18n` 校验六语言集合、偏好迁移、键集合、插值参数、消息语法和 Vue 模板。
...
- AI 与训练：模型统一使用 GGUF，不随安装包发布；AI 默认关闭，仅处理右侧用户选区。所有 AI 执行入口统一检查 AI 开关与已应用模型，条件不足时置灰禁用并通过 hover/键盘聚焦提示前往设置处理。LoRA 训练每次绑定一个经校验的基础 GGUF 模型。
