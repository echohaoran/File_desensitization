...
## 2026-09-09 v0.1.19 Beta 发布准备

- 用户授权提交新版本。五处版本统一为 0.1.19，使用新标签 v0.1.19。
- 包含规则批量启停/删除、三项固定字段保护、自动检测遵循管理配置、校对导航、顶部连续选区处理及预览位置保持。

## 2026-09-09 预览位置保持

- 删除当前检测项不再调用自动居中导航；三面板独立滚动。
- 预览片段携带原文起止偏移，检测项变更前捕获两栏可见原文字符及屏幕 Y，渲染后按原文位置补偿滚动；屏蔽浏览器默认滚动锚定与重排期间 hover 抢占。

## 2026-09-09 顶部批量选区操作

- 顶部排列取消脱敏、校对计数、脱敏、添加字段；预览标记点击只选中校对项，取消须主动点击顶部按钮。
- 右侧连续框选形成队列，显示数量与可移除的文本标签，可清空、合并重复/重叠区间；一次确认处理所有选区。
- 使用预览 DOM 偏移映射原文，兼容标记长度变化、重复文本和 DOCX 块偏移；左侧选区不参与。新增关键词按值去重，保留已有配置并使用随机规则 ID。

## 2026-09-09 预览标记取消脱敏

- 两栏检测标记支持点击（及 Enter）打开取消确认，弹窗显示对应原文，提供保留脱敏/取消脱敏；旧 DOCX 块预览点击也使用同一确认。
- 存在文字选区时不打开弹窗，避免干扰右侧框选。确认取消使用现有列表删除与相邻校对定位；已确认的旧生成状态随取消失效，需重新确认生成。

## 2026-09-09 逐项校对导航

- 预览标题下方新增左侧上一个、右侧下一个及当前位置计数；按正文位置排列检测项，同步定位与高亮三面板。
- 点击后沿用两秒 hover 保护，保留独立校对位置；删除当前项自动定位相邻项，重新开始清理导航状态；支持六语。

## 2026-09-09 固定高敏字段

- 手机号、身份证、银行卡号使用稳定 ID 实现不可删除保护，旧删除记录加载时恢复；保存和导入缺失字段时保留现有配置与启停状态。
- UI 显示固定字段标记并移除其单项删除入口；批量删除仅计算可删除项，仅选择固定项时禁用删除。全选仍可批量启停固定项。

## 2026-09-09 自动检测统一规则来源

- 移除前端无条件 PATTERNS 与图片模拟框；文本自动检测仅调用管理页已启用规则。
- 兼容后端停止追加预设正则、Presidio 和姓名检测；detect API 接收规则配置。空规则默认零结果。
- 前端补齐受规则控制的身份证日期/校验码、银行卡 Luhn、信用代码校验；后端算法使用一致候选格式和现有校验器。
- 新增 `scripts/check-rule-authority.mjs` 回归，覆盖空/停用/单项/自定义/算法/删除重载。

## 2026-09-09 敏感字段批量删除

- 批量编辑栏新增删除按钮，未选中时禁用；确认弹窗显示删除数量，取消保留原规则与选择，确认后清空选择并同步列表。
- 删除内置规则时同时持久化删除标记；第二次存储写入失败时回滚第一次写入并报告失败。六种语言补齐删除提示。

## 2026-09-09 敏感字段批量启停

- 默认列表改为 🟢 启用、🔴 停用，附带可访问说明及悬停提示。
- “正则转换”左侧新增编辑/完成按钮，编辑模式显示独立选择框、全选（含部分选中状态）、选中数量和批量启停操作。完成时清空选择，勾选本身不改变启用状态。
- 批量操作一次持久化，成功后更新列表，失败时保留原状态并明确提示；覆盖六种语言。
- 六语校验、浏览器批量启停与刷新持久化测试通过；本地签名打包及签名验证通过，已启动最新应用。尚未推送本轮功能。

## 2026-09-09 v0.1.18 发布与本机签名修复

- 本机原有签名密钥存在，旧命令未加载；新增安全构建包装脚本自动读取并核对公钥，保留 CI 环境变量优先。
- 六语功能随 0.1.18 发布，五处版本字段同步；本地 Tauri 完整构建成功，更新包签名使用应用现有公钥验证通过。
- 用户已授权提交、打标签及推送；使用新标签 v0.1.18，目标 GitHub，未复用旧标签。

## 2026-09-09 语言范围调整

- 按用户要求仅保留 zh/en/fr/ja/de/ko，移除俄语、阿拉伯语并补齐日语、德语、韩语全部翻译。
- 语言注册表集中到 `src/i18n/locales.js`；旧偏好按系统受支持语言迁移，默认回退英语，启动时恢复 LTR。
- 六语校验、生产构建和新增三语页面冒烟通过；最新本地 `.app` 和 DMG 已重新生成并启动。更新签名仍受本机未配置私钥限制，未推送。

## 2026-09-09 前端五语国际化

- 接入 vue-i18n，新增顶部语言切换与本机偏好保存，覆盖 zh/en/fr/ru/ar。语言包位于 `src/i18n/modules/`，支持具名动态插值与 Intl 日期/数量格式。
- 全部六个页面、全局弹窗、AI 按钮提示及共享错误消息完成接入；内置规则名称/说明在显示层翻译，用户内容和持久化协议保持原样。
- 新增 `npm run check:i18n`；浏览器已验证五语页面、阿拉伯文方向、语言保存、文件原文和自定义规则不受切换影响，修复了窄窗口长文案裁切和 CSS 空状态中文遗漏。
- 本轮不升版本或推送，等待本地测试包验证。
- 翻译校验最终为 578 键 × 5 语言；vue-i18n 固定为 `11.3.0`，兼容现有 Node 20 CI。已重新生成并启动包含本轮改动的 `0.1.17` 本地应用和 DMG，区别于已发布的同版本旧 Release；更新器签名阶段仍缺少本地私钥。

## 2026-09-07 交接

- 已核验 `v0.1.17` GitHub 构建成功、Release 正式发布且三平台安装包与签名更新清单齐全。
- `docs/handoff.md` 新增优先交接快照，说明当前提交、远端配置、测试证据及本地旧包版本限制。当前无新的业务实现任务。
...
## 2026-09-02 CNB 与 Gitee 标签自动打包

- 新增 `.cnb.yml`：标签推送会触发 Linux x64 Tauri DEB/AppImage 构建，自动创建同名 CNB Release、上传两个安装包和 SHA-256 校验和。
...
## 2026-09-03 AI 功能按钮统一禁用与引导

- 新增统一 AI 可用性读取与按钮门控组件，覆盖脱敏页 AI 全文检测、敏感字段页正则转换以及转换弹窗内的开始/重新转换。
- AI 关闭时按钮置灰且不可点击，hover 或键盘聚焦提示前往设置开启；AI 已开启但未应用模型时同样禁用，并提示前往设置下载并应用模型。
- 设置页在 AI 开关、应用模型、取消当前模型或刷新清单清除失效模型时广播状态变化，已打开页面可立即同步按钮状态。
- `npm run build` 和 ego-browser 两状态/三入口冒烟测试通过；最新 `.app` 与 ARM64 DMG 已生成并启动。更新器签名阶段因本机未注入私钥结束，不影响本地应用与 DMG 验收。

## 2026-09-06 v0.1.17 Beta 发布

- 用户确认提交、创建新标签并推送；功能提交为 `fd32ec7`，内容为 AI 开关与已应用模型对所有 AI 执行入口的统一门控。
- 前端、npm 锁文件、Tauri 配置、Rust crate 与 Cargo 锁文件版本统一提升至 `0.1.17`，新标签使用 `v0.1.17`，不复用已有 `v0.1.16`。
- 仓库仅保留 `.github/workflows/tauri-build.yml` 标签构建，标签发布目标为 GitHub Actions 的 macOS ARM64、Windows x64 和 Linux x64 Beta 安装包。
