...
## 2026-09-09 v0.1.19 Beta

- 本轮发布此前本地验收的规则管理、固定字段、统一检测与校对交互修复，版本统一到 0.1.19。
- 发布目标为 GitHub main 与新标签 v0.1.19；三平台打包由标签触发，不复用旧发布标签。

## 2026-09-09：v0.1.18 签名发布

- 发布中英法日德韩六语功能；本地签名缺失已修复为自动加载现有本机密钥，CI 签名变量保持兼容。
- 最新 0.1.18 `.app`、ARM64 DMG、更新归档与 `.sig` 完整生成，构建退出码 0，更新归档通过现有公钥验证。下方旧签名限制记录已解决。
- 新标签 v0.1.18；用户已授权 GitHub 推送以触发三平台构建。

## 2026-09-09：最新语言范围

- 用户将语言范围调整为中文、英语、法语、日语、德语、韩语；此范围替代下方历史五语记录。俄语和阿拉伯语已从菜单及语言包移除。
- 六语 578 键完整校验、新增语言各页面浏览器检查与旧偏好迁移通过。当前仍为本地 `0.1.17` 开发包，尚未推送发布。

## 2026-09-09：本地多语言实现（待用户验证）

- 前端接入中文、英文、法文、俄文、阿拉伯文，顶部选择器、本机偏好、动态提示及 RTL；语言包与测试入口见 `src/i18n/` 和 `scripts/check-i18n.mjs`。
- 578 键 × 五语校验及浏览器回归通过；虚构文件和自定义规则在切换语言时保持原样。vue-i18n 11.3.0 与现有 Node 20 CI 兼容。
- 本地最新 `.app` 和 ARM64 DMG 已包含这些变更，仍标记 `0.1.17 Beta`，尚未发布；GitHub 现有 `v0.1.17` 不包含本轮 i18n。
- 本地更新器签名缺少私钥；应用和 DMG 已生成并启动。用户确认后再推送，若发布必须递增版本并创建新标签。
...
- ego-browser 已覆盖 AI 关闭、AI 开启但无模型、模型就绪三种状态；前端构建通过，最新本地 `.app` 与 ARM64 DMG 已生成并启动。
- 发布版本统一为 `0.1.17`；本轮将提交版本变更、创建 `v0.1.17` 标签并显式推送到 GitHub，触发三平台 Tauri Release 工作流。
