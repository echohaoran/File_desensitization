...
- 原因：现有私钥位于用户目录，但旧 `tauri build` 命令未注入签名环境变量。
- 修复：`npm run tauri:build` 经过 `scripts/build-tauri.mjs` 自动加载受限本地密钥，并核对配对公钥；CI 显式变量仍优先。
- 验证：v0.1.18 完整打包退出码 0，生成更新归档及 `.sig`，归档签名通过应用内原公钥验证。
