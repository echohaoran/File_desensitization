...
## 2026-09-09 v0.1.19 发布验证

- 六语 609 键及规则/固定字段回归通过；`npm run tauri:build` 完整退出码 0，生成最新 macOS 应用、0.1.19 ARM64 DMG、更新归档与签名。
- 更新归档与受信任注释签名经应用现有公钥校验通过，最新应用已启动。使用新标签 v0.1.19 触发 GitHub 三平台构建。

## 2026-09-09 预览重排位置保持

- 长虚构文本含 100 个检测项，原文/脱敏面板分别独立滚动至 1500/1800px；选中标记后取消脱敏，检测项变为 99，两栏原文锚点 Y 位移均为 0px。
- 前端构建与 `--bundles app` 签名构建通过；本轮更新归档验签通过，已启动最新应用。未生成新 DMG，未推送。

## 2026-09-09 顶部操作与选区队列

- 六语 609 键校验通过。ego-browser 使用虚构 Alpha 重复文本和 Beta，累计两个选区后原检测数仍为 1；统一脱敏后变为 3，第二处 Alpha 被替换、第一处保持原文。
- 点击已有标记没有自动弹窗，顶部取消操作确认后检测数减少为 2；旧选区悬浮菜单不再出现。
- 最终代码追加验证左侧选区不入队、右侧重复文本位置精确映射，顶部添加字段后关键词存储为原文 Alpha。
- `npm run tauri:build -- --bundles app` 成功生成最新应用与签名归档，验签通过并启动；未生成本轮 DMG，未推送。

## 2026-09-09 预览标记取消弹窗

- 六语 603 键校验通过。ego-browser 上传三个虚构手机号，点击原始栏标记打开对应原文确认，保留后检测数仍为 3；点击脱敏栏标记并确认取消后检测数为 2，对应手机号恢复显示。
- 默认打包在 DMG 步骤失败；随后 `npm run tauri:build -- --bundles app` 成功生成本轮 `.app` 与签名更新归档，重新验签通过并启动应用。旧 DMG 不代表本轮变更，未推送。

## 2026-09-09 校对上一个/下一个

- 六语 600 键校验通过。ego-browser 上传三个虚构手机号文本，确认下一项依次为 1/3、3/3，返回上一项为 2/3；当前项有三处关联高亮，第一项上一个禁用、末项下一个禁用。
- 本地 Tauri 签名打包与归档验签通过，已启动最新应用，未推送。

## 2026-09-09 固定字段保护

- 回归验证三项固定字段在单项删除、批量删除、空规则导入中保留；旧版全部删除的存储能恢复三项；原停用状态不变。
- 六语校验通过；浏览器独立测试来源批量删除后刷新，确认仅剩手机号、身份证、银行卡，均无删除按钮；全选三项后批量删除禁用。
- 本地签名打包和更新包验证通过，最新应用已启动。未推送本轮修改。

## 2026-09-09 自动检测配置权威性

- Node 回归验证空规则、全停用、仅手机号、自定义正则、算法校验、删除全部后重载、新增自定义规则。
- Python 使用实际服务方法（跳过 NLP 初始化）验证空规则、停用、自定义唯一匹配与银行卡算法匹配；全部通过。
- ego-browser 从独立 5193 来源全选删除管理规则后上传虚构文本，验证零检测且左右正文一致。
- 本地 Tauri 完整签名打包、更新包签名验证通过；已启动最新应用供用户验证，未推送。

## 2026-09-09：批量删除验证

- 六语 594 键校验通过。内存存储测试覆盖内置与自定义规则混合删除、全部删除、重新加载，以及第二次写入失败时回滚第一次写入。
- ego-browser 使用独立 5192 测试来源验证全选、删除确认数量、取消保留列表、确认删除后清空列表与选择、刷新后内置规则不复现。
- Tauri 签名打包及归档签名验证通过；已启动最新本地应用供用户验证，未推送本轮改动。

## 2026-09-09：敏感字段状态圆点与批量编辑

- 六语 588 键校验、前端生产构建通过。ego-browser 验证默认列表复选框数量为 0，状态为 🟢；进入编辑后未选中时批量按钮禁用。
- 单选后全选框呈部分选中态；停用一项后 UI 与存储均只有一项停用。全选停用后全部显示 🔴，刷新仍保留停用状态且编辑选择框隐藏。
- 再次进入编辑、全选启用、点击完成后恢复 🟢 且复选框隐藏。测试浏览器中的原全启用状态已恢复。
- `npm run tauri:build` 完整成功，更新归档签名验证通过，最新 `.app` 已启动供用户测试。当前本地 0.1.18 包包含此改动，已推送的旧标签不包含本轮改动。

## 2026-09-09：v0.1.18 本地签名验收

- `npm run check:i18n`：578 键 × 6 语言通过；Tauri 完整构建退出码 0，生成 `.app`、DMG、`.app.tar.gz` 和 `.sig`。
- 现有本机公钥与应用配置一致；`verify-updater-signature.mjs` 对实际归档执行 Ed25519 校验，验证通过。未输出或提交私钥材料。
...
- `npm run check:i18n`：578 个键、5 种语言、8 个 Vue 组件通过；覆盖缺失键、消息编译、占位符一致性、静态翻译引用及模板编译。
- 五种语言分别访问首页、脱敏、还原、敏感字段、设置；验证语言偏好跨导航/刷新保留、RTL 方向、页面标题、无裸露翻译键。额外检查桌面测试页、空检测状态、AI 禁用提示、法文 760px 窄窗口与阿拉伯文布局。
- 使用含中英阿文本、花括号和邮箱的虚构文件完成上传确认；英→阿切换前后正文逐字一致。自定义规则包含 `{name}`、`@example` 和多语值，切换后存储内容逐字一致。
- 前端生产构建通过；依赖锁定 vue-i18n 11.3.0（Node >=16），兼容现有 Node 20 流水线。保留既有 PDF.js 警告。
- Tauri 生成最新 `.app` 和 9,388,292 字节 ARM64 DMG，并执行 `open -n` 启动应用。打包最后因缺少 `TAURI_SIGNING_PRIVATE_KEY` 返回 1；未声称生成有效更新签名，也未执行真实模型推理测试。
- 安装包：`src-tauri/target/release/bundle/dmg/文件脱敏与还原工具_0.1.17_aarch64.dmg`。待用户验证原生桌面交互；本轮未推送。
...
## 2026-09-01 还原完成结果弹窗烟测

- 环境：Vite `http://127.0.0.1:5174`、ego-browser、虚构文本映射。
- 结果：通过。完成还原后出现模态结果窗口，窗口内显示恢复后的 `13800138000` 与 5 个格式下载按钮；页面工作区没有残留底部下载栏。关闭弹窗后，上传的待还原文件仍保留。

## 2026-09-01 敏感字段删除同步烟测

- 环境：Vite `http://127.0.0.1:5174`、ego-browser 隔离任务空间；仅注入虚构自定义规则“删除验证字段”。
- 结果：通过。点击删除后出现应用内确认窗口；确认删除后，自定义规则从页面和 `desens_sensitive_rules` 中同时移除，列表由 `24 项` 更新为 `23 项`。再删除内置“手机号”后，页面同步移除该行、`desens_deleted_builtin_rules` 记录 `phone`，列表更新为 `22 项`。
- 打包：签名 `npm run tauri:build -- --bundles dmg,app` 通过；最新 `.app`、ARM64 DMG、`.app.tar.gz` 及 `.sig` 均非空，最新版应用已启动。

## 2026-09-01 敏感字段铺满布局烟测

- 环境：Vite `http://127.0.0.1:5174`、ego-browser 隔离任务空间，1476×995 浏览器视口。
- 结果：通过。页面工作区宽度为 1476px；左侧新增规则卡片为 389px，右侧已配置字段列表为 962px（2.47:1），右侧列表保持卡片内部纵向滚动。
- 打包：签名 `dmg,app` 打包通过，最新 macOS 应用已启动，DMG、更新归档和签名文件均为非空。

## 2026-09-01 还原页自动历史匹配烟测

- 环境：Vite `http://127.0.0.1:5174`、ego-browser 隔离任务空间；样本为 `trash/smoke-ui/restore-auto-match.txt` 与 `restore-no-match.txt`，仅含虚构占位符。
- 范围：初始左右布局、上传确认、唯一历史自动匹配、匹配失败保留上传文件、手动历史兜底。
...
## 2026-09-02 v0.1.15 发布标签前检查

- 范围：前端、Tauri、Cargo 与锁文件版本同步，以及标签触发发布前构建。
- 结果：通过。所有版本标记均为 `0.1.15`，`npm run build` 与 `git diff --check` 通过；将使用新标签 `v0.1.15`，不复用指向旧提交的 `v0.1.14`。

## 2026-09-02 v0.1.16 发布标签前检查
...
## 2026-09-06 v0.1.17 发布前检查

- 范围：前端、npm 锁文件、Tauri、Rust crate 与 Cargo 锁文件版本一致性，GitHub 标签触发器和发布提交指向。
- 结果：通过。五处版本均为 `0.1.17`，`npm run build`、`cargo check --manifest-path src-tauri/Cargo.toml` 与 `git diff --check` 通过；仓库不存在 `.cnb.yml`/`.gitee-ci.yml`，GitHub 工作流保留 `v*` 标签触发器。Rust 仅报告既有未使用类型警告。
