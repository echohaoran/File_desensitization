  ...
  <div class="container settings-page">
    ...
    <section class="settings-card">
      ...
      <div class="model-grid">
        <article v-for="model in recommendedModels" :key="model.id" class="model-item"><div class="model-item__body"><div class="model-item__top"><div><strong>{{ model.name }}</strong><span>{{ model.description }}</span><small>GGUF · {{ provider === 'modelscope' ? 'ModelScope' : 'Hugging Face' }} · 预计占用 {{ model.size }}</small></div><a class="btn btn--secondary btn--sm" :href="downloadUrl(model)" target="_blank" rel="noopener" @click="startDownload(model)">{{ downloadState[model.id] === 'done' ? '已打开' : '打开下载' }}</a></div><div v-if="downloadState[model.id] === 'downloading'" class="download-progress"><div class="download-progress__label"><span>正在准备下载…</span><span>{{ downloadProgress[model.id] }}%</span></div><div class="download-progress__track"><div class="download-progress__bar" :style="{ width: `${downloadProgress[model.id]}%` }"></div></div></div><div v-else-if="downloadState[model.id] === 'done'" class="download-done">已完成下载页面跳转，请将 GGUF 文件保存到本机</div></div></article>
      </div>
...
export default {
  name: 'Settings',
  data() {
    return { aiEnabled: localStorage.getItem('desens_ai_enabled') === 'true', provider: localStorage.getItem('desens_model_provider') || 'modelscope', modelPath: '', models: [], registering: false, modelError: '', lastTask: null, stopTaskListener: null, isTauri: isTauriRuntime(), downloadProgress: {}, downloadState: {}, downloadTimers: {}, recommendedModels: [
  ...
  methods: {
    ...
    startDownload(model) { if (this.downloadState[model.id] === 'downloading') return; this.downloadState[model.id] = 'downloading'; this.downloadProgress[model.id] = 8; clearInterval(this.downloadTimers[model.id]); this.downloadTimers[model.id] = setInterval(() => { const next = Math.min(92, (this.downloadProgress[model.id] || 0) + Math.floor(Math.random() * 16) + 7); this.downloadProgress[model.id] = next; if (next >= 92) { clearInterval(this.downloadTimers[model.id]); this.downloadState[model.id] = 'done' } }, 500) },
