...
export default {
  ...
  methods: {
    ...
    async startDownload(model) { if (!this.isTauri || this.downloadState[model.id] === 'downloading') return; const url = this.downloadUrl(model); if (!url) return; this.modelError = ''; this.downloadState[model.id] = 'downloading'; this.downloadProgress[model.id] = 1; clearInterval(this.downloadTimers[model.id]); this.downloadTimers[model.id] = setInterval(() => { if (this.downloadState[model.id] === 'downloading') this.downloadProgress[model.id] = Math.min(92, (this.downloadProgress[model.id] || 1) + 3) }, 500); try { const result = await downloadModel({ schema_version: 1, url, filename: model.hf.split('/').pop().split('?')[0] }); clearInterval(this.downloadTimers[model.id]); this.downloadProgress[model.id] = 100; this.downloadState[model.id] = 'done'; this.models = [...this.models, result.data] } catch (error) { clearInterval(this.downloadTimers[model.id]); this.downloadState[model.id] = 'error'; this.downloadProgress[model.id] = 0; this.modelError = error.message || '模型下载失败' } },
    ...
    applyModel(model) { this.activeModelId = model.id; localStorage.setItem('desens_active_model_id', model.id); localStorage.setItem('desens_active_model_path', model.path); this.modelError = '' },
