<template>
  <div class="container settings-page">
    ...
    <section class="settings-card">
      ...
      <div v-if="models.length" class="registered-models"><div v-for="model in models" :key="model.id" class="registered-model"><div class="registered-model__head"><div><strong>{{ model.name }}</strong><span>{{ model.status }} · {{ formatSize(model.size_bytes) }} · {{ model.sha256?.slice(0, 12) || '无校验值' }}…</span></div><div class="registered-model__actions"><button class="btn btn--secondary btn--sm" :class="{ 'is-active-model': activeModelId === model.id }" @click="applyModel(model)">{{ activeModelId === model.id ? '当前使用' : '应用模型' }}</button><button class="btn btn--ghost btn--sm" @click="removeModel(model)">取消登记</button></div></div><code>{{ model.path }}</code></div></div><div v-else class="settings-empty">暂无已登记模型</div>
    </section>
...
export default {
  ...
  methods: {
    ...
    saveAiSetting() { localStorage.setItem('desens_ai_enabled', String(this.aiEnabled)); announceAiAvailabilityChange(); this.notify(this.aiEnabled ? 'AI 脱敏已开启' : 'AI 脱敏已关闭') },
    ...
    async refreshModels() { if (!this.isTauri) { this.notify('请在桌面应用中刷新本地模型'); return }; try { const response = await listModels(); this.models = response.data?.items || []; if (!this.models.some(model => model.id === this.activeModelId)) { this.activeModelId = ''; localStorage.removeItem('desens_active_model_id'); localStorage.removeItem('desens_active_model_path'); announceAiAvailabilityChange() } this.notify(`模型清单已刷新，共 ${this.models.length} 个`) } catch (error) { this.modelError = error.message; this.notify(`刷新失败：${this.modelError}`) } },
    ...
    applyModel(model) { this.activeModelId = model.id; localStorage.setItem('desens_active_model_id', model.id); localStorage.setItem('desens_active_model_path', model.path); announceAiAvailabilityChange(); this.modelError = ''; this.notify(`已应用模型：${model.name}`) },
    async removeModel(model) { if (!this.isTauri) return; const accepted = await requestAppConfirm({ title: '取消登记模型', message: `将从本机模型清单移除“${model.name}”，不会删除本地文件。`, confirmText: '确认取消登记', tone: 'warning' }); if (!accepted) return; try { await unregisterModel({ schema_version: 1, model_id: model.id }); this.models = this.models.filter(item => item.id !== model.id); if (this.activeModelId === model.id) { this.activeModelId = ''; localStorage.removeItem('desens_active_model_id'); localStorage.removeItem('desens_active_model_path'); announceAiAvailabilityChange() } this.notify(`已取消登记模型：${model.name}`) } catch (error) { this.modelError = error.message || '取消登记失败'; this.notify(`取消登记失败：${this.modelError}`) } },
