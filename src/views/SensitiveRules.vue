<template>
  <div class="container rules-page">
    ...
    <section class="rules-list" aria-label="敏感字段规则列表"><div class="rules-list__head"><strong>已配置字段</strong><span>{{ rules.length }} 项</span></div><div v-if="!rules.length" class="empty-state"><p>尚未创建规则。</p></div><article v-for="rule in rules" :key="rule.id" class="rule-item" :class="{ 'is-disabled': !rule.enabled }"><label class="rule-toggle"><input type="checkbox" v-model="rule.enabled" @change="persist" /><span>{{ rule.enabled ? '已启用' : '已停用' }}</span></label><div class="rule-item__content"><strong>{{ rule.name }}</strong><span class="badge">{{ methodName(rule) }}</span><code>{{ rule.value }}</code></div><div class="rule-item__actions"><button class="text-btn" @click="editRule(rule)">编辑</button><button class="icon-btn" @click="removeRule(rule)" :aria-label="`删除 ${rule.name}`" title="删除">×</button></div></article></section>
  </div>
</template>
...
export default {
  name: 'SensitiveRules',
  data: () => ({ rules: loadSensitiveRules(), draft: emptyDraft(), editingId: '', error: '' }),
  computed: {
    valueLabel() {
