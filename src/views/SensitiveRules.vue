<template>
  <div class="container rules-page">
    ...
    <div v-if="editingId" class="rules-confirm-overlay" @click.self="cancelEdit">
      <section class="rules-confirm rules-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-rule-title">
        <p class="mono-label">EDIT RULE</p><h2 id="edit-rule-title">编辑敏感字段</h2>
        <div class="form-grid"><label>规则名称<input v-model.trim="draft.name" /></label><label>识别方式<select v-model="draft.kind"><option value="name">姓名</option><option value="keyword">关键词</option><option value="regex">正则表达式</option><option value="algorithm">算法校验</option><option value="nlp">NLP / 上下文</option></select></label><label class="form-grid__wide">{{ valueLabel }}<input v-model.trim="draft.value" :placeholder="placeholder" /></label></div>
        <p v-if="error" class="rules-error">{{ error }}</p><div class="rules-confirm__actions"><button class="btn btn--secondary" @click="cancelEdit">取消</button><button class="btn btn--primary" @click="saveRule">保存修改</button></div>
      </section>
    </div>
  </div>
</template>
