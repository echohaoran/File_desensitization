...
const INPUT = path.join(__dirname, 'test-input-after-add.txt');
...
async function run() {
  ...
  // Step 1: select "张三" in first paragraph and add it
  await page.evaluate(() => {
  ...
  // Step 2: select "成长一号" in second paragraph and check popup
  ...
  console.log('Second paragraph selection after first add:', popupVisible);

  await browser.close();
  fs.unlinkSync(INPUT);

  if (!popupVisible.popupVisible) {
    console.error('Popup did not show for second paragraph selection after first add');
    process.exit(1);
  }
